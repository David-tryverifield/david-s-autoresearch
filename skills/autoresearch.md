...
- **Ceiling case:** If all criteria are tied at the maximum score, write "Ceiling achieved — no weak point. No rewrite required." and proceed through Steps 5–8 unchanged. Do not skip any steps.
...
```
Before writing, verify each score in this log matches the corresponding value in `evals/[skill-name]/set-[N]/eval-run-[M].md`. Fix any discrepancy before saving.
...
- **Ceiling case:** If Step 4 identified a ceiling, write "No rewrite — skill at ceiling." Do not modify the skill file. Still proceed to Step 8.

**Step 8 — Report table (required)**
At the end of each set's run output, append the following table before looping to the next set:
...
|-----|-----------|-------|-------------|
| [N] | Q[X] or "ceiling" | X/25 | baseline or +X vs prior set |
```

- This table must match the data in `set-summary.md` exactly.
- Only include sets that have been fully scored. Do not add a row for the current set if scoring is pending.
- **Ceiling case:** Use `Weakest Q = ceiling` and `Score = max/max` (e.g. 15/15 for 3 reps).

---

### After the loop
Report a summary table covering all sets:
